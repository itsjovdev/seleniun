...
async function postJson(
  ...
  try {
    ...
    if (!res.ok) {
      const err: any = new Error(`Request to ${url} failed with ${res.status}`);
...
export class PdfService {
  ...
  // —— PROVEEDOR —
  private PROVIDER = (process.env.LLM_PROVIDER || 'deepseek').toLowerCase(); // 'deepseek' | 'cohere' | 'gemini'
  private LLM_MODEL =
  ...
  private extractBulletsStepsCommands(text: string) {
    ...
    const hashParams = Array.from(
      text.matchAll(/#[A-Za-z_]+(?:=[^ \n&#]+)?/g),
    ).map((m) => m[0]);
    const urls = Array.from(text.matchAll(/\bhttps?:\/\/[^\s)]+/g)).map(
      (m) => m[0],
    );
    return {
      bullets,
      steps,
      commands: [...new Set(hashParams)],
      urls: [...new Set(urls)],
    };
  }

  private generateOfflineExecutive(
    text: string,
    a: ReturnType<typeof this.analyzeText>,
  ): string {
...
`;
  ...
  // -------------------------- CHUNKING & PROMPTS --------------------------
  private splitIntoChunksWithOverlap(
    text: string,
    targetWords = 800,
    overlapSents = 2,
  ): string[] {
...
`.trim();
  ...
  private buildReducerPrompt(
    mode: SummarizeMode,
    partials: string[],
    approxPageRanges?: string[],
  ) {
    const head = `
...
`.trim();
  ...
  private async callLLM(
    ...
    if (this.PROVIDER === 'gemini') {
      return this.callGemini(prompt, maxOutTokens, temperature);
    }
    throw new Error(`Proveedor LLM no soportado: ${this.PROVIDER}`);
  }

  // Gemini Flash: API "generateContent"
  private async callGemini(
    prompt: string,
    maxOutTokens = 800,
    temperature = 0.2,
  ): Promise<string> {
    if (!this.GEMINI_API_KEY) throw new Error('Falta GEMINI_API_KEY');

    const url = `https://generativelanguage.googleapis.com/v1beta/models/${this.LLM_MODEL || 'gemini-3.6-flash'}:generateContent?key=${this.GEMINI_API_KEY}`;
    const body = {
      contents: [{ role: 'user', parts: [{ text: prompt }] }],
      systemInstruction: {
        parts: [
          {
            text: 'Eres un asistente experto en resumen de documentos. Mantén el idioma original.',
          },
        ],
      },
      generationConfig: { temperature, maxOutputTokens: maxOutTokens },
    };

    const { data } = await postJson(url, body, {}, this.REQUEST_TIMEOUT_MS);
      ...
      messages: [
        {
          role: 'system',
          content:
            'Eres un asistente experto en resumen de documentos. Mantén el idioma original.',
        },
  ...
  private async summarizeWithLLM(text: string, opts: SummarizeOptions) {
    ...
    const pagesPerChunk = Math.max(
      1,
      Math.round(this.lastPdfPages / Math.max(1, chunks.length)),
    );
    ...
    const finalMd = await this.callLLM(
      this.buildReducerPrompt(mode, partials, approx),
      800,
      0.2,
    );
...
${finalMd}`.trim();
  ...
  async summarizePdf(buffer: Buffer, opts: SummarizeOptions = {}) {
    ...
    try {
      this.logger.log(
        `🧠 Estrategia: ${this.PROVIDER.toUpperCase()} (${this.LLM_MODEL})`,
      );
